# v2.0.2

- Added optional nonce to charts.

# v2.0.1

- values in y axis are not stacked.

# v2.0.0

- updated to @vcmap/core and @vcmap/ui Version 6.0
- balloon feature info view
  - remove shadow effect in line and bar graph
  - add table to feature info view
  - remove table header in feature info
  - remove month in graph title

# v1.0.4

- fixes an issue with plugin config editor
- added color selector to plugin config editor
- added i18n translation for charts and colors

# v1.0.3

- added support of thematic surface settings in plugin editor
- added support of thematic surfaces representation in balloon

# v1.0.2

- added translation of editor
- added bar chart option

# v1.0.1

added plugin editor for app-configurator

# v1.0.0

Document features and fixes
